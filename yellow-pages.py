...
def process_hardcoded_file(file_path, column_name):
    ...
    urls = data[column_name].dropna().tolist()[149:200]
    industries = data.get('Industry', pd.Series(['Unknown'] * len(data))).tolist()[149:200]
    job_titles = data.get('Job Title', pd.Series(['Unknown'] * len(data))).tolist()[149:200]

    # Process each URL
    for idx, (url, industry, job_title) in enumerate(zip(urls, industries, job_titles), start=150):
