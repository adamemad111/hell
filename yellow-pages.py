...
def process_hardcoded_file(file_path, column_name):
    ...
    industries = data.get('Industry', pd.Series(['Unknown'] * len(data))).tolist()[749:800]
    job_titles = data.get('Job Title', pd.Series(['Unknown'] * len(data))).tolist()[749:800]

    # Process each URL
    for idx, (url, industry, job_title) in enumerate(zip(urls, industries, job_titles), start=750):
