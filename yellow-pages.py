...
def process_hardcoded_file(file_path, column_name):
    ...
    urls = data[column_name].dropna().tolist()[1199:1250]
    industries = data.get('Industry', pd.Series(['Unknown'] * len(data))).tolist()[1199:1250]
    job_titles = data.get('Job Title', pd.Series(['Unknown'] * len(data))).tolist()[1199:1250]
    # Process each URL
