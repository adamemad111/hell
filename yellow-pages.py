...
def process_hardcoded_file(file_path, column_name):
    ...
    urls = data[column_name].dropna().tolist()[1249:1300]
    industries = data.get('Industry', pd.Series(['Unknown'] * len(data))).tolist()[1249:1300]
    job_titles = data.get('Job Title', pd.Series(['Unknown'] * len(data))).tolist()[1249:1300]
    # Process each URL
    for idx, (url, industry, job_title) in enumerate(zip(urls, industries, job_titles), start=1250):
